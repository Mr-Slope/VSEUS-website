...
import { TicketEmailForm } from '@/components/portal/TicketEmailForm';
import { getSessionUser } from '@/lib/session';
import { getMemberById, getMyTickets } from '@/db/queries';
...
export default async function ProfilePage() {
  ...
  const [user, tickets] = await Promise.all([
    getMemberById(sessionUser.id),
    getMyTickets(sessionUser.id),
  ]);
  if (!user) redirect('/auth/login');

  const registeredEvents = tickets.map((t) => t.event);
  ...
  return (
    <div className="p-6 lg:p-8 max-w-2xl">
      ...
      {/* Ticket email */}
      <TicketEmailForm currentTicketEmail={user.ticketEmail} />
